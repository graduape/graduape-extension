class GraduApePopup {
  ...
  showApiKeySetup() {
    ...
    tokenList.innerHTML = `
      <div style="text-align: center; padding: 20px; color: #a0aec0;">
        <h3 style="color: #48bb78; margin-bottom: 16px;">🔑 API Key Required</h3>
        ...
        <p style="margin-bottom: 16px; font-size: 12px;">
          1. Go to <a href="https://moralis.io" target="_blank" style="color: #48bb78;">moralis.io</a><br>
        ...
        <input type="text" id="apiKeyInput" placeholder="Enter your Moralis API key" 
               style="width: 100%; padding: 8px; margin-bottom: 12px; background: #2d3748; border: 1px solid #4a5568; color: #fff; border-radius: 8px;">
        <button id="saveApiKeyBtn" style="background: #48bb78; color: #fff; border: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-weight: 600;">
          Save API Key
        </button>
      </div>
    `
  ...
  updateConnectionStatus(connected) {
    ...
    if (connected) {
      ...
      refreshBtn.title = "Connected to API"
      refreshBtn.style.background = "#48bb78"
      this.hideError()
      console.log("API connected, tokens received:", this.tokens.length)
    } else {
      refreshBtn.textContent = "✗"
      refreshBtn.title = "Disconnected - Click to reconnect"
      refreshBtn.style.background = "#f56565"
      if (!this.needsApiKey) {
  ...
  updateFilterStats() {
    ...
    const qualityCount = this.tokens.filter(
      (token) => token.holders >= 50 && token.marketCap >= 50000 && token.volume24h >= 10000,
    ).length
    ...
    statsElement.innerHTML = `
      Totals: ${totalTokens} | Filtered: <span class="filter-count">${filteredCount}</span> | Quality: <span class="filter-count">${qualityCount}</span>Risk: <span class="risk-okay">${okayCount} Okay</span> | <span class="risk-risky">${riskyCount} Risky</span> | <span class="risk-analyzing">${analyzingCount} Analyzing</span>
    `
  }
    ...
    if (this.filteredTokens.length === 0) {
      tokenList.innerHTML = `
        <div style="text-align: center; padding: 40px; color: #a0aec0;">
          ...
          <br><br>
          <small>🔄 Polling API every 45 seconds</small>
          <br>
  ...
  createTokenElement(token) {
    ...
    // Create badges based on token properties
    let badges = '<span class="badge migrated">MIGRATED</span>'
    if (isQuality) {
      badges += '<span class="badge high-quality">HIGH QUALITY</span>'
    }
    if (riskAnalysis.level === "analyzing") {
      badges += '<span class="badge analyzing">ANALYZING</span>'
    } else if (riskAnalysis.level === "risky") {
      badges += '<span class="badge risky">RISKY</span>'
    } else if (riskAnalysis.level === "okay") {
      badges += '<span class="badge okay">OKAY</span>'
    }
    ...
    div.innerHTML = `
      ...
      <div class="token-stats">
        <div class="token-stat">
          ...
          <div class="token-stat-value">$${this.formatNumber(token.marketCap)}</div>
        </div>
        <div class="token-stat">
          <div class="token-stat-label">24h Volume</div>
          <div class="token-stat-value">$${this.formatNumber(token.volume24h)}</div>
        </div>
        <div class="token-stat">
          <div class="token-stat-label">24h Change</div>
          <div class="token-stat-value ${priceChangeClass}">+${Math.abs(token.priceChange24h).toFixed(2)}%</div>
        </div>
      </div>
    `
